...
interface ChurchEngagementScoreBreakdown {
  total: number;
  visit: {
    points: number;
    max: number;
...
function visitFollowUpFact(date: string | null, daysOverdue: number | null) {
  if (!date) return "No visit scheduled";
  if (daysOverdue !== null && daysOverdue > 0) return `${pluralizeDays(daysOverdue)} overdue`;
  if (daysOverdue === 0) return "Visit due today";
  return `Visit scheduled ${formatDate(date)}`;
}

export function ChurchEngagementScore({ churchId, score }: ChurchEngagementScoreProps) {
  const [open, setOpen] = useState(false);
  const [loading, setLoading] = useState(false);
  const [breakdown, setBreakdown] = useState<ChurchEngagementScoreBreakdown | null>(null);
  ...
  async function handleOpen() {
    ...
    const { data, error: rpcError } = await supabase.rpc("get_church_engagement_score_breakdown", {
    ...
    setBreakdown(data as ChurchEngagementScoreBreakdown);
  }
  ...
  const rows = breakdown
    ? [
        {
          label: "Visit",
          points: breakdown.visit.points,
          max: breakdown.visit.max,
          fact: recencyFact(
            "Visited",
            breakdown.visit.last_visit_date,
            breakdown.visit.days_since,
            "No visits recorded",
          ),
        },
        {
        ...
        },
        {
          ...
          fact: visitFollowUpFact(
            breakdown.follow_up.next_visit_date,
