    ...
    async function fetchData() {
      try {
        const [
          ...
          supabase.from("project_staff").select("staff_id").eq("project_id", id),
        ]);

        setStaff(staffData || []);
        setSelectedStaffIds(
          ((projectStaffData || []) as Array<{ staff_id: string }>).map(
            row => row.staff_id,
          ),
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedStaffIds.length > 0) {
        const { error: insertError } = await supabase.from("project_staff").insert(
  ...
  if (fetching) {
    return (
      <Card className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-on-surface-variant">Loading project data...</p>
      </Card>
    );
  }
        ...
        <div className="pl-2">
          <div className="mb-8">
            ...
            <p className="mt-2 text-sm text-on-surface-variant">
              Update project goals, timeline, funding, and team ownership.
            </p>
          </div>
